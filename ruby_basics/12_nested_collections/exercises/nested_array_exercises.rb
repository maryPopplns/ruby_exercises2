def blank_seating_chart(number_of_rows, seats_per_row)
  ...
  rows.map {|row| Array.new(seats_per_row)}
end
...
def count_empty_seats(chart)
  ...
  chart.reduce(0) {|total, current| total + current.count(nil)}
end
...
def find_favorite(array_of_hash_objects)
  ...
  array_of_hash_objects.select {|object| object[:is_my_favorite?] == true}[0]
end
